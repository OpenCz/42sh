...
static int handle_input_char(main_t *stock_main, buffer_t *buff,
    int *cursor, input_ctx_t *ctx)
{
    if (ctx->ch == 3) {
        my_putstr("\n");
        display_prompt(stock_main->czshrc->prompt, ctx->user);
        return 0;
    }
    if (ctx->ch == 12)
        return handle_ctrl_l(ctx->stock_main, ctx->user);
    if (ctx->ch == 4)
        return handle_ctrl_d(buff->len, ctx->user, stock_main);
    if (ctx->ch == ARROW_START)
        return arrow_handling(stock_main->history,
            buff->buffer, cursor, buff->len);
...
static int check_char(main_t *stock_main, buffer_t *buff,
    int *cursor, input_ctx_t *ctx)
{
    if (read(STDIN_FILENO, &ctx->ch, 1) == -1)
        return -1;
    return handle_input_char(stock_main, buff, cursor, ctx);
}

static int create_command(main_t *stock_main, char **buffer,
    input_ctx_t *ctx)
{
    int len = 0;
    int status = 0;
    ...
    history_t *history, char *user)
{
    ...
    int create_cmd = create_command(stock_main, buffer, &ctx);
